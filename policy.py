...
class CnnPolicySVIB(object):
    ...
    def __init__(self, sess, ob_space, ac_space, nbatch, master_ts, worker_ts, cell=256,
        ...
        channels = 1
        stddev = 0.1 # the variance of noise distribution
        wX = tf.placeholder(tf.uint8, [nbatch, nh, nw, nc])#worker obs
        istraining = tf.placeholder(tf.bool, shape=[])
        # 参数的设置需要加入５%的噪声，增加一个维度来进行extend
        ...
        with tf.variable_scope("model", reuse=reuse):
            activ = tf.nn.relu
            # 改变数据的类型uint8 -> tf.float32
            wX_ = tf.cast(wX, tf.float32) / 255.

            #　命名是为了方便管理，第一层是对图片进行encoder，然后通过一个全连接层，分成一个ｐolicy网络和一个ｖalue网络。
            def encoder(X, noise_, scope = 'ordinary_encoder', reuse=reuse):
                with tf.variable_scope(scope, reuse=reuse):
                    h1 = activ(conv(X, 'c1', nf=16, rf=8, stride=4,init_scale=np.sqrt(2)))  # h, w = 20, 20
                    if algo != 'regular':
            ...
            if model == 'train_model':
                ...
                rpf_matrix, rpf_grads = rpf_kernel(wvf_expand, rpf_h)

                def Gibbs_Dist(X, noise_, scope='Gibbs_module', reuse=reuse):
                    with tf.variable_scope(scope, reuse=reuse):
                        gibbs_1 = activ(conv(X, 'g1', nf=16, rf=8, stride=4, init_scale=np.sqrt(2)))  # h, w = 20, 20
                        gibbs_1_ = tf.concat([gibbs_1, noise_], axis=-1)
                        gibbs_2 = activ(
                            conv(gibbs_1_, 'g2', nf=32, rf=4, stride=2, init_scale=np.sqrt(2)))  # h, w = 9, 9
                        gibbs_3 = activ(
                            conv(gibbs_2, 'g3', nf=64, rf=3, stride=2, init_scale=np.sqrt(2)))  # h, w = 4, 4
                        gibbs_4 = activ(
                            conv(gibbs_3, 'g4', nf=64, rf=3, stride=1, init_scale=np.sqrt(2)))  # h, w = 2, 2
                        gibbs_5 = conv_to_fc(gibbs_4)
                        gibbs_6 = fc(gibbs_5, 'ener_fc', 1)
                        energy_function = tf.nn.sigmoid(gibbs_5)
                    return energy_function

                energy = Gibbs_Dist(wX_, noise)
                ...
                def T(X, H, scope='T', reuse=reuse):
                    '''
                    function：visualization of mutual information using algorithm MINE
                    :param X:
                    :param H:
                    :param scope:
                    :param reuse:
