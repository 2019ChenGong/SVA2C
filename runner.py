...
from utils import np_compute_cosine, np_l2norm1, compute_ir, obs_preprocess, mse
...
class Runner(object):
    def __init__(self, env, model, nsteps=8, gamma=0.99, use_vae=True):
        self.env = env
        self.model = model
        nh, nw, nc = env.observation_space.shape
        nenv = env.num_envs
    ...
    def run(self):
        ...
        for n in range(self.nsteps):
            if self.use_vae:
                actions, values, states, _ = self.model.single_layer_step(self.whs, self.states, self.dones)
            else:
                actions, values, states, _ = self.model.single_layer_step(self.obs, self.states, self.dones)
            b_obs.append(np.copy(self.obs))
            b_whs.append(np.copy(self.whs))
            b_actions.append(actions)
            b_values.append(values)
            ...
            if self.use_vae:
                show_img(self.model.embedding_decoder(h)[0]/200., scope='decoder')
        ...
        b_dones = b_dones[:, 1:]
        if self.use_vae:
            last_values = self.model.value(self.whs, np.zeros(shape=self.whs.shape, dtype=self.whs.dtype), self.states, self.dones).tolist()
        else:
            last_values = self.model.value(self.obs, np.zeros(shape=self.whs.shape, dtype=self.whs.dtype), self.states, self.dones).tolist()
        #discount/bootstrap off value fn
        # raw_er = 0.
        raw_er = np.sum(b_rewards[0])
        for n, (rewards, dones, value) in enumerate(zip(b_rewards, b_dones, last_values)):
            rewards = rewards.tolist()
            dones = dones.tolist()
            # raw_ir += discount_with_dones(rewards, dones, self.gamma)[0]
        ...
        return b_obs, b_whs, b_states, b_rewards, b_masks, b_actions, b_values, raw_er
